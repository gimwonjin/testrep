...
// 함수형 컴포넌트: 컴퍼넌트를 함수형으로 만든것(변수형으로도 만들수 있다.)
// 함수형 컴포넌트를 만들때에는 함수명의 첫 글자는 반드시 대문자여야 한다
// 함수형 컴포넌트 안에서는 JSX 문법으로 만드 리렉트 엘리먼트를 리턴해줘야 한다.
// for ==> htmlFor, onclick/onblurr ==> onClick/onBlur
...
function App() {
  ...
  // 순수 자바스크립트로 작성한다면 주사위 이미지마다 화면을 만들거나, 비동기로 화면에 요소를 추가한다, 삭제 하는 코드를 작성해야한다.
  // 리액트 애서는 State라는것을 사용한다.
  // 이 State가 바뀔 때마다 리액트가 알라서 화면을 새로 렌더링한다
